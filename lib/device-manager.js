...
function getSimctlData() {
	...
	logger.i("getSimctlData out =" + out);
	return out.toString(); //can be buffer object
}

function DeviceManager() {

	logger.i("DEVICEMANAGER STARTUP @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@   pid=" + process.pid);
	var self = this;
	...
	logger.i('typeof simctlData =' + typeof simctlData);
...
DeviceManager.prototype.findByTag = function (tag) {
	...
	logger.i('DeviceManager.findByTag(' + tag + ') =' + result);
	if (result === undefined) {
		logger.i("RETURNING NULL. Dumping devicesByTag");
		this.devicesByTag.forEach(function(value, key, map){
			logger.i("key="+ key + " value=" + JSON.stringify(value));
		});
		return null;
	}
	return result;
	/*
	logger.i('DeviceManager.findByTag(' + tag + ') =' + JSON.stringify(result));
	var debugResult = (result ? result : null);
	logger.i('DeviceManager debugResult=' + JSON.stringify(debugResult));
	//this.devicesByTag.forEach(function(value, key, map){
	//	logger.i("key="+ key + " value=" + JSON.stringify(value));
	//})
