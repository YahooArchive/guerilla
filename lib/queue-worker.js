...
QueueWorker.prototype.remove = function (id, callback) { //TODO:bruceg remove callback parm?
		...
		kue.Job.get(id, function (error, job) {
			...
			job.complete(function () {
				...
				async.series([
					...
					function(cb) {
						qloggerD("QueueWorker.remove:46 about to searchQueue");
						self.searchQueue();
						cb(null);
					}
				])
...
QueueWorker.prototype.isRunnable = function (id, callback) {
	...
	kue.Job.get(id, function (error, kjob) {
		...
		if (kjob.state()==='complete') {
			logger.i('QueueWorker.isRunnable ERROR. Received job that is already compelte:'+ kjob.toJSON());
