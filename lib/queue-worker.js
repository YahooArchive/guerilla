	...
	if (self.jobs[id]) {
		...
		kue.Job.get(id, function (error, job) {
			...
			job.remove(function (err) {
				qloggerD("QueueWorker.remove 35 err=" + err);
...
QueueWorker.prototype.isRunnable = function (id, callback) {
	...
	kue.Job.get(id, function (error, kjob) {
		...
		if (kjob.state()==='complete') {
			logger.w('QueueWorker.isRunnable Warning. Received job that is already complete:'+ JSON.stringify(kjob.toJSON()));
...
QueueWorker.prototype.process = function (job, callback) {
	...
	job.active(function (error) {
		qloggerD("QueueWorker.process: 1 for job:" + job.id);
		if (error) {
			logger.i('QueueWorker.process job.active returned error' + error);
...
QueueWorker.prototype.shutdown = function (callback) {
	qloggerD('QueueWorker.shutdown entered');
	var self = this;
