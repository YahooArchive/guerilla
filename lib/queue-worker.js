...
QueueWorker.prototype.remove = function (id) {
...
QueueWorker.prototype.isRunnable = function (id, callback) {
	...
	kue.Job.get(id, function (error, kjob) {
		...
		if (kjob.state()==='complete') {
			logger.i('QueueWorker.isRunnable ERROR. Received job that is already complete:(constructor)'+ kjob.constructor);
			logger.i('QueueWorker.isRunnable ERROR. Received job that is already complete:'+ JSON.stringify(kjob.toJSON()));
			logger.i('QueueWorker.isRunnable ERROR. Received job that is already complete:'+ kjob);
...
QueueWorker.prototype.process = function (job, callback) {
	var self = this;
	qloggerD("QueueWorker.process for id" + job.id);
	qloggerD(job.toJSON());
	qloggerD('setting removeOnComplete(true)');
	//job.removeOnComplete(true).save(); //TODO:bruceg 1. use callabck. 2. make this a queue option or an option when the job is created
