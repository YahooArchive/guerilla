...
QueueWorker.prototype.remove = function (id, callback) {
	logger.d("QueueWorker.remove 1 id=" + id);
	var self = this;

	if (self.jobs[id]) {
		logger.d("QueueWorker.remove 5 found id " + id);
		master.stopWorkers(self.jobs[id].job_id);
		delete self.jobs[id];
		logger.d("QueueWorker.remove 10: deleting job:" + id);
		kue.Job.get(id, function (error, job) {
			if (error || !job) {
				logger.d('QueueWorker.remove 20: ERROR getting id:' + id);
				return (callback || function () {})();
			}
			logger.d("QueueWorker.remove 30 about to set job.complete");
			job.complete(function () {
				logger.d("QueueWorker.remove 40 job.complete in callback");
