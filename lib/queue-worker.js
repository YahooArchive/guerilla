...
QueueWorker.prototype.remove = function (id, callback) {
	console.log("QueueWorker.remove id=" + id);
	var self = this;

	if (self.jobs[id]) {
		console.log("QueueWorker: found id " + id);
		master.stopWorkers(self.jobs[id].job_id);
		delete self.jobs[id];
		console.log("QueueWorker: deleted job:" + id);
...
QueueWorker.prototype.process = function (job, callback) {
	var self = this;
	console.log("QueueWorker.process for id" + job.id);

	job.active(function (error) {
		console.log("QueueWorker.process: 1");
		if (error) {
			console.log("QueuWorker process: 1.5- ERROR ERROR. Returning");
			return;
		}
		console.log("QueueWorker.process: 2");
		self.jobs[job.id] = job.data;
		self.jobs[job.id].platform = job.type;
		console.log("QueueWorker.process: 3-forking worker");
		master.forkWorker(job.data, function () {
			console.log("QueueWorker.process: 4-removing id:" + job.id);
			self.remove(job.id);
		});
