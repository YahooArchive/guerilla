...
JobMaster.prototype.forkWorker = function (data, callback) {
	...
	Job.findById(job_id, function (error, job) {
		...
		logger.d("JobMaster.forkWorker about to fork for pid" + process.pid);
		logger.trace();
		var worker = cp.fork(
		...
		if (!self.workers[job_id])
			self.workers[job_id] = [];
		logger.d("JobMaster.forkworker self.workers[" + job_id + "].push(" + worker.pid +")" );
		...
		worker.on('exit', function (code, signal) {
			logger.d("JobMaster.forkWorker.worker.on(exit) entered");
